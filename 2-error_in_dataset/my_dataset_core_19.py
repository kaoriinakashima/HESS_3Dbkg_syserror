...
class Dataset(abc.ABC):
    """Dataset abstract base class which is able to fit and evaluate Nuisance parameters as well.
    ...
    #def stat_sum_G(self):
    #    """Total statistic given the current model parameters and nuisance parameters with Gaussian Part (depending on correlation matrix)."""
    #    G = self.stat_array_G()
    #    return self.stat_sum() + G
...
class Datasets(collections.abc.MutableSequence):
    ...
    def parameters(self):
        ...
        for dataset in self:
            if hasattr(dataset, 'N_parameters'):
                if dataset.N_parameters is not None:
    ...
    #    """Unique nuisance parameters (`~gammapy.modeling.Parameters`).
    #    TODO: use N parameter from all datasets not only [0] 
    #    """
    ...
    #def stat_sum_G(self):
    #    """Compute joint likelihood with Gaussian part (Nuisance * correlation matrix * nuisance) """
    #    stat_sum_G = 0
    #    for dataset in self:
    #        stat_sum_G += dataset.stat_sum_G()
    #    return stat_sum_G
