...
class MapDatasetNuisance(MapDataset):
    ...
    inv_corr_matrix : `~numpy.array`
        Inverted correlation matrix to describe the correlations between the nuisance parameters. Needs to have same dimension as nuisance parameters.
    ...
    def __init__(
        ...
        self.models = models
        self.meta_table = meta_table
        self.inv_corr_matrix = inv_corr_matrix  
    ...
    def __str__(self):
        ...
        #stat_sum_G = np.nan
        #if self.inv_corr_matrix is not None and self.N_parameters is not None:
        #    stat_sum_G = self.stat_sum_G()
        
        #str_ += "\t{:32}: {:.2f}\n\n".format(
        #    "Fit statistic value (-2 log(L) + G)", stat_sum_G
        #)

        n_models, n_pars, n_free_pars = 0, 0, 0
        if self.models is not None:
            n_nui_pars = len(self.N_parameters)
            n_free_nui_pars = len(self.N_parameters.free_parameters)

        str_ += "\t{:32}: {}\n".format("Number of Nuisance parameters", n_nui_pars)
        str_ += "\t{:32}: {}\n\n".format("Number of free Nuisance parameters",  n_free_nui_pars)    
        return str_.expandtabs(tabsize=2)    
    ...
    def geoms(self):
        ...
        if self.nuisance_mask is not None:
            ...
            fac = int(self.background.data.shape[1] / shape_N)
            geoms["geom_down"] = self._geom.downsample(fac)
    ...
    def to_hdulist(self):
        ...
        if self.nuisance_mask is not None:
            nuisance_mask_int = self.nuisance_mask.copy()
            nuisance_mask_int.data = nuisance_mask_int.data.astype(int)
            hdulist += nuisance_mask_int.to_hdulist(hdu="nuisance_mask")[exclude_primary]
            
        if self.inv_corr_matrix is not None:
            geom_matrix = WcsGeom.create(npix = np.shape(self.inv_corr_matrix)[0])
            inv_corr_matrix_map = Map.from_geom(geom_matrix, data = self.inv_corr_matrix)
            hdulist += inv_corr_matrix_map.to_hdulist(hdu="inv_corr_matrix")[exclude_primary]
    ...
    def from_hdulist(cls, hdulist, name=None, lazy=False, format="gadf"):
        ...
        if "NUISANCE_MASK" in hdulist:
            nuisance_mask = Map.from_hdulist(hdulist, hdu="nuisance_mask")
            nuisance_mask.data = nuisance_mask.data.astype(bool)
            kwargs["nuisance_mask"] = nuisance_mask
            
        if "INV_CORR_MATRIX" in hdulist:
            kwargs["inv_corr_matrix"] =  Map.from_hdulist(hdulist, hdu="inv_corr_matrix").data
    ...
    def slice_by_idx(self, slices, name=None):
        ...
        if self.nuisance_mask is not None and self.nuisance_mask is not None and  self.inv_corr_matrix is not None:
            slice_mask  = self.geoms['geom_down'].energy_mask(
                    self.geoms['geom_down'].axes[0].edges[slices['energy'].start],
                    self.geoms['geom_down'].axes[0].edges[slices['energy'].stop]).data
            slice_mask = (slice_mask  & self.nuisance_mask.data)[self.nuisance_mask.data].flatten()
            N_slice    = [ self.N_parameters[i]  for i in range(len(self.N_parameters.value)) if slice_mask.data[i]]
            N_slice    = Parameters(N_slice)
            kwargs["N_parameters"] = N_slice
            kwargs["nuisance_mask"] =  self.nuisance_mask.slice_by_idx(slices=slices)
            kwargs["inv_corr_matrix"] = self.inv_corr_matrix[slice_mask][:,slice_mask]
